...
{
    /// <summary>
    ///     The Handler abstract class.  Every class which inherits from this will be responsible for a kind of request for the
    ///     restaurant.
    /// </summary>
        ...
        public override void ProcessRequest(PurchaseOrder purchase)
        {
            if (purchase.Price < 1000)
                Console.WriteLine("{0} approved purchase request #{1}",
                    GetType().Name, purchase.RequestNumber);
            else if (Supervisor != null) Supervisor.ProcessRequest(purchase);
        }
    }
    ...
    internal class PurchaseOrder
    {
        ...
        public PurchaseOrder(int number, double amount, double price, string name)
        {
            ...
            Console.WriteLine("Purchase request for " + name + " (" + amount + " for $" + price +
                              ") has been submitted.");
        }
